          ...
          <table className="w-full text-left border-collapse min-w-[680px]">
            <thead>
              <tr className="border-b border-[#F0F0F0]">
                <th className="px-4 py-3 text-[10px] tracking-[0.15em] uppercase text-muted font-medium w-16 text-center">Ikon</th>
                <th className="px-4 py-3 text-[10px] tracking-[0.15em] uppercase text-muted font-medium">Layanan</th>
                <th className="px-4 py-3 text-[10px] tracking-[0.15em] uppercase text-muted font-medium w-40">Harga Mulai</th>
                <th className="px-4 py-3 text-[10px] tracking-[0.15em] uppercase text-muted font-medium w-56">Fitur Unggulan</th>
              ...
              {services.map((service) => (
                <tr key={service.id} className="hover:bg-surface-hover transition-colors">
                  <td className="px-4 py-3 text-center">
                    <span className="inline-block text-[11px] bg-surface-hover text-muted px-2 py-1">{service.icon}</span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-[12px] text-muted">{service.priceFrom || "Hubungi Kami"}</td>
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1">
                      ...
                      {Array.isArray(service.features) && service.features.length > 3 && (
                        <span className="text-[10px] px-1.5 py-0.5 bg-surface-hover text-muted border border-border">+{service.features.length - 3}</span>
                      )}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <div className="inline-flex gap-1.5">
                      <button onClick={() => handleOpenEdit(service)} className="p-1.5 border border-border hover:border-border text-muted hover:text-foreground transition-colors">
                        <Edit2 size={13} strokeWidth={1.8} />
                      </button>
                      <button onClick={() => handleDelete(service.id)} className="p-1.5 border border-border hover:border-red-300 text-muted hover:text-red-500 transition-colors">
        ...
        <div className="fixed inset-0 z-50 bg-black/20 backdrop-blur-[2px] flex justify-end" onClick={() => setDrawerOpen(false)}>
          <div className="w-full max-w-lg h-full bg-surface border-l border-border flex flex-col overflow-y-auto shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave} className="flex-1 px-6 py-5 space-y-4 text-[13px]">
              <div>
                <label className="block text-[10px] tracking-[0.15em] uppercase text-muted mb-1.5">Nama Layanan</label>
              ...
              <div>
                <label className="block text-[10px] tracking-[0.15em] uppercase text-muted mb-1.5">Ikon (Lucide)</label>
              ...
              {/* Features */}
              <div>
                <label className="block text-[10px] tracking-[0.15em] uppercase text-muted mb-1.5">Fitur Layanan</label>
              ...
              <div>
                <label className="block text-[10px] tracking-[0.15em] uppercase text-muted mb-1.5">Harga Mulai (Opsional)</label>
                <input value={priceFrom} onChange={(e) => setPriceFrom(e.target.value)} type="text"
                  className="w-full px-3 py-2.5 bg-background border border-border text-foreground text-[13px] focus:outline-none focus:border-border transition-colors"
                  placeholder="Rp 5.000.000" />
              </div>
              <label className="flex items-center gap-2 cursor-pointer p-3 bg-background border border-border hover:border-border transition-colors w-fit">
