    ...
    <div className="space-y-6 max-w-6xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-6 border-b border-border">
        <div>
          ...
          <h1 className="text-2xl font-semibold text-foreground tracking-tight">Testimonials</h1>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {testimonials.map((t) => (
            <div
              key={t.id}
              className={`bg-surface border border-border p-5 hover:border-border transition-colors relative ${!t.isPublished ? "opacity-60" : ""}`}
            >
              {!t.isPublished && (
                <span className="absolute top-4 right-4 text-[10px] tracking-[0.12em] uppercase bg-surface-hover text-muted px-2 py-0.5">Draft</span>
              )}
              ...
              <div className="pt-4 border-t border-[#F0F0F0] flex items-center justify-between gap-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-[13px] font-medium text-foreground">{t.clientName}</p>
                    <p className="text-[10px] text-muted">{t.clientRole} · {t.company}</p>
                  </div>
                </div>
                <div className="flex gap-1.5">
                  <button onClick={() => handleOpenEdit(t)} className="p-1.5 border border-border hover:border-border text-muted hover:text-foreground transition-colors">
                    <Edit2 size={12} strokeWidth={1.8} />
                  </button>
                  <button onClick={() => handleDelete(t.id)} className="p-1.5 border border-border hover:border-red-300 text-muted hover:text-red-500 transition-colors">
        ...
        <div className="fixed inset-0 z-50 bg-black/20 backdrop-blur-[2px] flex justify-end" onClick={() => setDrawerOpen(false)}>
          <div className="w-full max-w-lg h-full bg-surface border-l border-border flex flex-col overflow-y-auto shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave} className="flex-1 px-6 py-5 space-y-4 text-[13px]">
              <div>
                <label className="block text-[10px] tracking-[0.15em] uppercase text-muted mb-1.5">Nama Klien</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] tracking-[0.15em] uppercase text-muted mb-1.5">Jabatan</label>
                  <input required value={clientRole} onChange={(e) => setClientRole(e.target.value)} type="text"
                    className="w-full px-3 py-2.5 bg-background border border-border text-foreground text-[13px] focus:outline-none focus:border-border transition-colors"
                    placeholder="CEO / PM" />
                </div>
                <div>
                  <label className="block text-[10px] tracking-[0.15em] uppercase text-muted mb-1.5">Perusahaan</label>
              ...
              <div>
                <label className="block text-[10px] tracking-[0.15em] uppercase text-muted mb-1.5">Avatar URL</label>
              ...
              <div>
                <label className="block text-[10px] tracking-[0.15em] uppercase text-muted mb-1.5">Ulasan / Feedback</label>
              ...
              <div>
                <label className="block text-[10px] tracking-[0.15em] uppercase text-muted mb-1.5">Rating (1–5)</label>
